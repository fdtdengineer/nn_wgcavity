...
class SymX(nn.Module):
    ...
    def forward(self, x):
        ...
        xq = torch.cat([torch.zeros(num_data, 1, 1, 2), x], dim=2)
        xq2 = xq.view(num_data, 2, -1, 2).transpose(2, 3)
        x_half = torch.cat([xq2.flip(dims=[3])[:,:,:,:-1], xq2], dim=3)
        x_all = torch.cat([x_half.flip(dims=[2]), torch.zeros(num_data, 2, 1, 13), x_half], dim=2)
        return x_all
...
if __name__ == "__main__":
    ...
    lambda_reg=2
    # Data Loadings
    df_q_fem = pd.read_csv(filepath_q_fem + filename_q_fem)
